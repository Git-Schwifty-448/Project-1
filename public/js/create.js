...
$(() => {
    ...
    $('button.submit')[0].addEventListener("click", event => {
        ...
        unprocessed_days = unprocessed_days.sort( (a,b) =>{
            console.log(b[0])
            console.log(a[0])
            console.log(new Date(b[0].date) - new Date(a[0].date))
        ...
        for (let i in unprocessed_days) {
            date_array.push(new Date(unprocessed_days[i][0]))
            time_array.push(unprocessed_days[i][1])

        }

        let processed_days = [date_array,time_array]
        console.log(processed_days)
        // processed_days = processed_days.sort((e1, e2) => (new Date(e1[0])).getTime() - (new Date(e2[0])).getTime())
